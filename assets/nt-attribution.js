...
(function () {
  ...
  function patch(link) {
    ...
    url.searchParams.set("start", START); // never lose the SendPulse code
    // source_channel: keep the link's own value (e.g. 'conf'); default only if absent.
    if (!url.searchParams.get("source_channel")) {
